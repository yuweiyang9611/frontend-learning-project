...
const importPreview = computed(() => {
  ...
  const newlyCompleted = pendingImport.value.days.filter((day) => day.completed && !current.get(day.day)?.completed).length;
...
onMounted(() => {
  try {
    ...
    if (stored) {
      ...
      else statusMessage.value = '已忽略损坏或旧版本的本地进度：' + decoded.error;
    }
  } catch {
...
function toggleDay(day) {
  ...
  day.completedAt = day.completed ? new Date().toISOString() : null;
  if (day.completed && day.minutes === 0) day.minutes = 120;
}

function normalizeMinutes(day) {
  const value = Number(day.minutes);
  day.minutes = Number.isInteger(value) ? Math.min(1440, Math.max(0, value)) : 0;
}
  ...
  <section class="learning-progress" aria-labelledby="learning-progress-title">
    <header class="learning-progress__header">
      <div>
        ...
        <p>记录完成状态、主动学习分钟数、复盘和验收链接；数据不会上传到服务器。</p>
      </div>
    ...
    <p class="learning-progress__status" role="status" aria-live="polite">{{ statusMessage }}</p>
        ...
        <div class="learning-progress__days">
          <article v-for="day in week.days" :key="day.day" class="learning-progress__day">
            <label class="learning-progress__check">
              <input type="checkbox" :checked="day.completed" @change="toggleDay(day)" />
              <strong>Day {{ String(day.day).padStart(2, '0') }}</strong>
            </label>
            <label>
              主动学习分钟
              <input v-model.number="day.minutes" type="number" min="0" max="1440" step="5" @blur="normalizeMinutes(day)" />
            </label>
            <label>
              HTTPS 或相对证据链接
              <input
            ...
            <label>
              当日复盘（最多 500 字）
              <textarea v-model="day.note" maxlength="500" rows="2" placeholder="卡点、验证和明天的第一步"></textarea>
            </label>
          </article>
        </div>
      </details>
