...
test("rejects duplicate, out-of-range, and incompatible imports", () => {
  ...
  assert.equal(
    decodeProgress({
      ...base,
      days: [{ day: 92, completed: false, completedAt: null }],
    }).ok,
    false,
  );
  assert.equal(
...
test("rejects a completed day without minutes, reflection, and evidence", () => {
  ...
  assert.equal(decodeProgress({ ...base, days: [incomplete] }).ok, false);
  const complete = {
    ...incomplete,
    note: "完成了独立变体，并用测试输出证明了行为。",
    evidence: [
      { label: "report", url: "./learning-evidence/day-01/report.json" },
    ],
  };
