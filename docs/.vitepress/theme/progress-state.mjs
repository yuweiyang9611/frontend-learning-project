...
export const PROGRESS_STORAGE_KEY =
  "issueflow:frontend-learning-project:progress:v1";
...
export function decodeProgress(input) {
  ...
  if (!Array.isArray(input.days) || input.days.length > DAY_COUNT)
    return { ok: false, error: "days 必须是最多 91 项的数组。" };
  const seen = new Set();
  const days = [];
  for (const candidate of input.days) {
    if (!candidate || typeof candidate !== "object" || Array.isArray(candidate))
      return { ok: false, error: "每个 Day 必须是对象。" };
    if (
      !Number.isInteger(candidate.day) ||
      candidate.day < 1 ||
      candidate.day > DAY_COUNT ||
      seen.has(candidate.day)
    ) {
      return { ok: false, error: "Day 必须在 1–91 且不能重复。" };
    }
    if (typeof candidate.completed !== "boolean")
      return { ok: false, error: "completed 必须是 boolean。" };
    if (!(
      candidate.completedAt === null ||
      typeof candidate.completedAt === "string"
    ))
      return { ok: false, error: "completedAt 必须是字符串或 null。" };
    const minutes = candidate.minutes ?? 0;
    const note = candidate.note ?? "";
    if (!Number.isInteger(minutes) || minutes < 0 || minutes > 1440)
      return { ok: false, error: "单日分钟数必须在 0–1440。" };
    if (typeof note !== "string" || note.length > 500)
      return { ok: false, error: "单日复盘最多 500 字。" };
    const evidence = candidate.evidence ?? [];
    if (!Array.isArray(evidence) || evidence.length > 5)
      return { ok: false, error: "每个 Day 最多 5 个证据链接。" };
    const safeEvidence = [];
    for (const item of evidence) {
      if (!item || typeof item !== "object" || Array.isArray(item))
        return { ok: false, error: "证据必须是对象。" };
      if (
        typeof item.label !== "string" ||
        item.label.length > 100 ||
        !isSafeEvidenceUrl(item.url)
      ) {
        return {
          ok: false,
          error: "证据标签或 URL 不合法；只允许 HTTPS 或本站相对地址。",
        };
      }
    ...
    if (
      candidate.completed &&
      (minutes < 120 || note.trim().length < 20 || safeEvidence.length === 0)
    ) {
      return {
        ok: false,
        error: `Day ${candidate.day} 标记完成时必须包含 120 分钟、20 字复盘和证据链接。`,
      };
    }
...
export function mergeProgress(current, imported, replace = false) {
  ...
  for (const next of imported.days) {
    ...
    byDay.set(next.day, {
      ...
      minutes: replace
        ? next.minutes
        : Math.max(previous.minutes, next.minutes),
  ...
  return {
    ...
    days: Array.from(
      { length: DAY_COUNT },
      (_, index) => byDay.get(index + 1) ?? emptyDay(index + 1),
    ),
  };
}
...
export function normalizeProgress(input) {
  ...
  return decoded.ok
    ? mergeProgress(createProgress(), decoded.value, true)
    : createProgress();
}
